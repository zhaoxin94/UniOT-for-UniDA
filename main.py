...
classifier = CLS(feature_extractor.output_dim,
                 cls_output_dim,
                 hidden_mlp=2048,
                 feat_dim=256,
                 temp=temp)
...
optimizer_featex = optim.SGD(feature_extractor.parameters(),
                             lr=args.train.lr * 0.1,
                             weight_decay=args.train.weight_decay,
                             momentum=args.train.sgd_momentum,
                             nesterov=True)
optimizer_cls = optim.SGD(classifier.parameters(),
                          lr=args.train.lr,
                          weight_decay=args.train.weight_decay,
                          momentum=args.train.sgd_momentum,
                          nesterov=True)
optimizer_cluhead = optim.SGD(cluster_head.parameters(),
                              lr=args.train.lr,
                              weight_decay=args.train.weight_decay,
                              momentum=args.train.sgd_momentum,
                              nesterov=True)
...
scheduler = lambda step, initial_lr: inverseDecaySheduler(
    step, initial_lr, gamma=10, power=0.75, max_iter=args.train.min_step)
...
cnt_i = 0
with TrainingModeManager([feature_extractor, classifier],
...
while global_step < args.train.min_step:
    ...
    for minibatch_id, ((im_source, label_source, id_source),
                       (im_target, _, id_target)) in enumerate(iters):
        ...
        nb_value_tt, nb_feat_tt = memqueue.get_nearest_neighbor(
            norm_feat_t, id_target.cuda())
        neighbor_candidate_sim = torch.cat(
            [nb_value_tt.reshape(-1, 1), feat_mat2], 1)
        values, indices = torch.max(neighbor_candidate_sim, 1)
        neighbor_norm_feat = torch.zeros(
            (minibatch_size, norm_feat_t.shape[1])).cuda()
        for i in range(minibatch_size):
            neighbor_candidate_feat = torch.cat(
                [nb_feat_tt[i].reshape(1, -1), norm_feat_t], 0)
        ...
        S_tt = torch.cat([after_cluhead_t, neighbor_output, mqfill_output_t],
                         0)
        S_tt *= temp
        ...
        for i in range(minibatch_size):
            sub_loss_local = 0
            sub_loss_local += -torch.sum(
                neighbor_Q[i, :] * F.log_softmax(after_cluhead_t[i, :]))
            sub_loss_local += -torch.sum(
                anchor_Q[i, :] * F.log_softmax(neighbor_output[i, :]))
            sub_loss_local /= 2
            loss_local += sub_loss_local
        loss_local /= minibatch_size
        loss_global = -torch.mean(
            torch.sum(anchor_Q * F.log_softmax(after_cluhead_t, dim=1), dim=1))
        ...
        if global_step > 100:
            ...
            # Adaptive filling
            newsim, fake_size = adaptive_filling(ubot_feature_t,
                                                 source_prototype, gamma, beta,
                                                 fill_size_uot)
            ...
            loss_CCD = criterion(after_lincls_t[high_conf_label_id, :],
                                 high_conf_label[high_conf_label_id])
        else:
            loss_CCD = 0
        ...
        with OptimizerManager(
            [opt_sche_featex, opt_sche_cls, opt_sche_cluhead]):
            loss_all.backward()
        ...
        if global_step % args.log.log_interval == 0:
            ...
            counter.addOneBatch(
                variable_to_numpy(one_hot(label_source, len(source_classes))),
        ...
        if global_step % args.test.test_interval == 0:
            results = eval(feature_extractor,
                           classifier,
                           target_test_dl,
                           classes_set,
                           gamma=gamma,
                           beta=beta)
            logger.add_scalar('cls_common_acc', results['cls_common_acc'],
                              global_step)
            logger.add_scalar('cls_tp_acc', results['cls_tp_acc'], global_step)
            logger.add_scalar('tp_nmi', results['tp_nmi'], global_step)
            logger.add_scalar('cls_overall_acc', results['cls_overall_acc'],
                              global_step)
...
if parser_args.dataset in ['office31', 'officehome', 'visda']:
    ...
    draw_tsne(feature_extractor, classifier, cluster_head, source_test_dl,
              target_test_dl, s_label, t_label, writer, parser_args.dataset)
