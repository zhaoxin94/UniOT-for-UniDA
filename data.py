...
# FileListDataset
source_train_ds = FileListDataset(list_path=source_txt,
                                  path_prefix=dataset_root,
                                  transform=train_transform,
                                  return_id=True,
                                  filter=(lambda x: x in source_classes))
target_train_ds = FileListDataset(list_path=target_txt,
                                  path_prefix=dataset_root,
                                  transform=train_transform,
                                  return_id=True,
                                  filter=(lambda x: x in target_classes))
source_test_ds = FileListDataset(list_path=source_txt,
                                 path_prefix=dataset_root,
                                 transform=test_transform,
                                 return_id=False,
                                 filter=(lambda x: x in source_classes))
target_test_ds = FileListDataset(list_path=target_txt,
                                 path_prefix=dataset_root,
                                 transform=test_transform,
                                 return_id=False,
                                 filter=(lambda x: x in target_classes))
...
class_weight = {
    x: 1.0 / freq[x] if args.data.dataloader.class_balance else 1.0
    for x in freq
}
...
source_train_dl = DataLoader(dataset=source_train_ds,
                             batch_size=batch_size,
                             sampler=sampler,
                             num_workers=num_workers,
                             drop_last=True)
target_train_dl = DataLoader(dataset=target_train_ds,
                             batch_size=batch_size,
                             shuffle=True,
                             num_workers=num_workers,
                             drop_last=True)
target_test_dl = DataLoader(dataset=target_test_ds,
                            batch_size=batch_size,
                            shuffle=False,
                            num_workers=num_workers,
                            drop_last=False)
# for memory queue init
target_initMQ_dl = DataLoader(dataset=target_train_ds,
                              batch_size=batch_size,
                              shuffle=True,
                              num_workers=num_workers,
                              drop_last=True)
# for tsne feature visualization
source_test_dl = DataLoader(dataset=source_test_ds,
                            batch_size=batch_size,
                            shuffle=False,
                            num_workers=num_workers,
                            drop_last=False)
