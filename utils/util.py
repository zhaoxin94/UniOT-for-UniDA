...
class MemoryQueue(nn.Module):
    def __init__(self, feat_dim, batchsize, n_batch, T=0.05):
        ...
        self.register_buffer('mem_feat', torch.zeros(self.queue_size,
                                                     feat_dim))
        self.register_buffer('mem_id', torch.zeros((self.queue_size),
                                                   dtype=int))
    ...
    def update_queue(self, features, ids):
        ...
        update memory queue
        """
        w_ids = torch.arange(self.next_write,
                             self.next_write + self.batchsize).cuda()
    ...
    def random_sample(self, size):
        ...
        """
        id_t = torch.floor(torch.rand(size) *
                           self.mem_feat.size(0)).long().cuda()
...
class ResultsCalculator(object):
    ...
    def __init__(self,
        ...
        self.overall_accs, self.overall_acc_aver = self._get_overall_acc(
            classes_set['source_classes'], label, predict_label,
            uniformed_index)
        common_accs = dict()
        ...
        self.h_score = 2 * self.common_acc_aver * self.tp_acc / (
            self.common_acc_aver + self.tp_acc)
        self.tp_nmi = self._nmi_cal(private_label, private_pred)
        self.h3_score = 3 * (
            1 / (1 / self.common_acc_aver + 1 / self.tp_acc + 1 / self.tp_nmi))

    def _get_overall_acc(self, source_classes, label, predict_label,
                         uniformed_index):
        unif_label = self._tplabel_unif(source_classes, label, uniformed_index)
        overall_accs, overall_acc_aver = self._recall_acc_cal(
            source_classes + [uniformed_index], predict_label, unif_label)
    ...
    def _clutser_acc_cal(self, common_classes, tp_classes, label, proto_pred):
        ...
        matrix, mat_index = self._hungarian_matching(transform_label,
                                                     proto_pred)
        sum_Ncorrect = 0
